...
describe("defaultLayout", () => {
  ...
  it("starts as the grid people already know", () => {
    const layout = defaultLayout(APPS);
    expect(layout.groups.map((g) => g.name)).toEqual(DEFAULT_GROUPS.map((g) => g.name));
  ...
  it("places every app exactly once", () => {
    const placed = defaultLayout(APPS).groups.flatMap((g) => g.items);
  ...
  it("gives every placement a key of its own", () => {
    const keys = defaultLayout(APPS).groups.flatMap((g) => g.items).map((i) => i.key);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it("starts everything the same size, shown, and unpinned", () => {
    for (const item of defaultLayout(APPS).groups.flatMap((g) => g.items)) {
...
describe("hiding, and getting things back", () => {
  ...
  it("brings back everything that was hidden or deleted", () => {
    ...
    const back = reconcile(restoreAll(layout), APPS);
...
describe("groups", () => {
  it("adds a group", () => {
    const layout = addGroup(defaultLayout(APPS), "Daily");
...
describe("reconcile", () => {
  ...
  it("places an app the stored layout never heard of", () => {
    const layout = defaultLayout(APPS.filter((a) => a.id !== "gmail"));
    const after = reconcile(layout, APPS);
  ...
  it("leaves a layout that is already right alone", () => {
    const layout = defaultLayout(APPS);
    expect(reconcile(layout, APPS)).toEqual(layout);
  });
});
...
describe("persistence", () => {
  ...
  it("falls back to the default when nothing is stored", () => {
    ...
    expect(shape(loadLayout(APPS))).toEqual(shape(defaultLayout(APPS)));
  });
  ...
  it("falls back rather than breaking on nonsense", () => {
    for (const junk of ["", "not json", "[]", '{"groups":"no"}', '{"version":99}']) {
      localStorage.setItem("jky.apps.layout", junk);
      expect(loadLayout(APPS).groups.length).toBeGreaterThan(0);
      expect(loadLayout(APPS).groups.flatMap((g) => g.items).length).toBe(APPS.length);
    }
  });

  it("reconciles what it loads, so a stored layout is never stale", () => {
    saveLayout(defaultLayout(APPS.filter((a) => a.id !== "gmail")));
    expect(loadLayout(APPS).groups.flatMap((g) => g.items).map((i) => i.appId)).toContain("gmail");
  });
});
